...
<nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
    <!-- Navbar Brand-->
    <a class="navbar-brand ps-3" href="{{route('accounts.index')}}">Myfin</a>
    <!-- Sidebar Toggle-->
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="createAccountForm">
                                    ...
                                    <div id="degradedBanner" class="alert alert-warning" style="display:none">
                                        Система перевантажена, будь ласка, спробуйте пізніше.
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <div class="card">
                            <div class="position-absolute top-0 end-0">
                                ...
                                <div class="modal fade" id="editModal{{ $account->id }}" tabindex="-1"
                                     aria-labelledby="editModalLabel{{ $account->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editModalLabel{{ $account->id }}">Редагувати
                                                    рахунок</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Закрити"></button>
                                            </div>
                                                ...
                                                <form method="POST"
                                                    ...
                                                    <div class="form-group mb-3">
                                                        ...
                                                        <input type="number" class="form-control"
                                                               id="Amount{{ $account->id }}" name="amount"
                                ...
                                <button type="submit" class="btn btn-info" data-bs-toggle="modal"
                                        data-bs-target="#NewTransactionModal{{ $account->id }}">
                                    додати транзакцію
                                </button>

                                <div class="modal fade" id="NewTransactionModal{{ $account->id }}" tabindex="-1"
                                     aria-labelledby="NewTransactionModalLabel{{ $account->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="NewTransactionModalLabel{{ $account->id }}">
                                                    додати нову транзакцію</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Закрити"></button>
                                            </div>
                                            <div class="modal-body" id="transactionsContainer">
                                                <form method="POST" class="js--transactionClassForm"
                                                      id="transactionForm{{$account->id}}"
                                                      action="{{ route('transactions.create') }}">
                                                    @csrf
                                                    @method('POST')
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio" name="radio"
                                                               id="radioDeposit{{ $account->id }}" value="deposit">
                                                        <label class="form-check-label"
                                                               for="radioDeposit{{ $account->id }}">
                                                            Поповнення
                                                        </label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio" name="radio"
                                                               id="radioSpend{{ $account->id }}" value="spend">
                                                        <label class="form-check-label"
                                                               for="radioSpend{{ $account->id }}">
                                                            Витрачення
                                                        </label>
                                                    </div>
                                                    ...
                                                    <div class="dropdown mt-2">
                                                        ...
                                                        <div class="js--categoryError disabled text-danger mt-1">
                                                            Виберіть категорію
                                                        </div>
                                                        <input type="hidden" name="category_id"
                                                               class="js--categoryInput">
                                                    </div>
                                                    ...
                                                    <div class="form-group mb-3">
                                                        ...
                                                        <input type="text" class="form-control"
                                                               id="desc{{$account->id}}" name="description">
                                                    </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            form.addEventListener("submit", async (e) => {
                ...
                try {
                    const resp = await window.apiFetch("/accounts", "POST", body);
                    const data = await window.handleApiResponse(resp);

                    console.log("RESULT:", data);

                    // Якщо сервер повернув HTML →
        ...
        document.querySelectorAll('input[name="radio"]').forEach(radio => {
            radio.addEventListener('change', async (event) => {
                ...
                try {
                    ...
                    // Перетворюємо відповідь у масив, фільтруючи requestId
                    const categories = Object.values(data).filter(item => typeof item === 'object');
